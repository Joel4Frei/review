class Marker:
    ...
    def help(self):
        ...
        print('3 Functions: \n',
              ...
              'pro("num","total") = Progress\n',
              '\t Prints progress: 1/3,2/3,3/3. Total can also be set using set_pro_max("max"), then it is pro("num")\n\n')
    ...
    def pro(self, cur, tot=False):

        cur_color = self.Colors['yellow']
        tot_col = self.Colors['blue']

        if not tot:
            tot = self.max

        print(f'{cur_color}{cur}{self.Colors["RESET"]}/{tot_col}{tot}{self.Colors["RESET"]}')

        if cur == tot:
            print(f'Reached {self.Colors["gold"]}END{self.Colors["RESET"]} of progress reviewing!')

    def set_pro_max(self,max):
        max_col = self.Colors['blue']
        self.max = max

        print(f'The maximum of the progress function is set to {max_col}{self.max}{self.Colors["RESET"]}.')
